...
I would say that I find EDA to be the more interesting approach overall. I started out as a psychology major, a field I know EDA lends itself particularly well to. Not only is it a more appropriate methodology for the highly variable messiness of analyzing human behavior, it's also less susceptible to issues like confirmation bias and false sense of certainty that afflicts the more traditional approach of CDA. 
In EDA, I’m interested in and agree with Tukey’s conceptualization of data analysis as detective work and a journey of exploration with the goal of revealing the underlying story that the data has to tell. I also definitely see Tukey’s argument against the more structured and rigorous approaches as creating a focus so narrow that the analyst misses what was at first more subtle perspectives, preemptively closing themselves off to potentially valuable insight. 
However, I certainly see the veracity of the argument made by some that EDA is not without its own set of drawbacks; its broad nature and focus on triangulating methodology rendering it of limited use in the later stages of Data Analysis bereft of the support of other approaches. Luckily however EDA is an able partner and perfect compliment to traditional CDA; the two forming a highly efficient and effective synergy.
Finally, what I would say draws me most to Tukey’s EDA philosophy is its application. The acquisition of knowledge begins by its very nature with that which is unknown, as such the central theme is necessarily discovery. In EDA Tukey presents a much needed alternative framework to traditional approaches.
...
Both rnorm and runif are functions in R that generate random numbers between a range based on distributions. Both functions require the user to specify the quantity of random numbers to be generated as function parameters. The ways in which they are different are that rnorm creates numbers for a normal distribution of a function using a specified mean and standard deviation with the numbers having probabilities along the normal distribution bell curve. The function runif creates numbers for a uniform distribution of a function using min and max with the numbers having an equal or uniform probability.

Similar to the runif function, the sample function takes in population size, sample size, and with or without replacement as function parameters and returns a random sample of observations from the population.

The function with parameters plot(x.rnorm, x.runif) is different from the functions with parameters plot(x.rnorm, (x.rnorm.sample = sample(x.rnorm)); and plot(x.runif, (x.runif.sample = sample(x.runif)); in this case because  plot(x.rnorm, x.runif) appears centrally distributed along the x.rnorm axis with few data points close to the x.rnorm mean, while being evenly distributed across the entirety of the x.runif axis. This appears in the shape of a “cone.”. The plot(x.rnorm, (x.rnorm.sample = sample(x.rnorm)) has a greater distribution along the x.rnorm axis and lesser along the sample(x.rnorm) axis and appears to the be the shape of an oval. Finally, the plot(x.runif, (x.runif.sample = sample(x.runif)) has a completely even distribution along all both axis and appears in the shape of a rectangle.
...
## your code goes here ...
# From CPTS 419 Discussion Board
nsim = 0;
x = numeric(length = 0)
i = 1
while (length(x) < 5)
...
#Errors with previous function workaround
sub.myrolls = subsetDataFrame(myrolls, "roll.total", "==", 23);

#sub.myrolls = myrolls[which(myrolls$roll.total == 23), ]
...
#sub.outcomes.df = outcomes.df[which(outcomes.df$roll.total == 23), ]
...
ANSWER: When throwing the 3 10-sided dice at the same time, the number of ways to roll a 23 is 36. The probability that a single throw results in a 23 is 0.036.
...
# probability of 12
roll_12 = myrolls[which(myrolls$roll.total == 12), ];

# probability of 22
roll_22 = myrolls[which(myrolls$roll.total == 22), ];

# results of above
roll_12_outcomes = outcomes.df[which(outcomes.df$roll.total == 12), ];
roll_22_outcomes = outcomes.df[which(outcomes.df$roll.total == 22), ];

# probability of rolling 12 or 22
roll_12_or_22_outcomes = roll_12_outcomes[1, 'prob'] + roll_22_outcomes[1, 'prob'];
cat(roll_12_or_22_outcomes, "\n");
...
ANSWER: The probability that you roll a 12 or 22 on a single throw is 0.1
...
# probability of 26
roll_26 = myrolls[which(myrolls$roll.total == 26), ];

## probability of 29
roll_29 = myrolls[which(myrolls$roll.total == 29), ];

# results of above
roll_26_outcomes = outcomes.df[which(outcomes.df$roll.total == 26), ];
roll_29_outcomes = outcomes.df[which(outcomes.df$roll.total == 29), ];

# probability of rolling 26 or 29
roll_26_or_29_outcomes = roll_26_outcomes[1, 'prob'] + roll_29_outcomes[1, 'prob'];
cat(roll_26_or_29_outcomes, "\n");
...
ANSWER: The probability that you roll a 26 or a 29 on a single throw is 0.018.
...
# probability of 3 on 1 throw
roll_3 = myrolls[which(myrolls$roll.total == 3), ];

# results of above
roll_3_outcomes = outcomes.df[which(outcomes.df$roll.total == 3), ];

# probability of rolling 3 twice in a row
roll_3_1x = roll_3_outcomes[1, 'prob'];

cat(roll_3_1x, "\n");

roll_3_2x = (roll_3_1x * roll_3_1x);
...
ANSWER: The probability that you roll a 3 on a single throw is 0.001. The probability that you roll a 3 twice in a row, where first throw = 3 **AND** second throw = 3 is 0.000001.
...
# looping thru range of outcomes
for (i in rolls)
{
  #print(i)
  outcome = outcomes.df[which(outcomes.df$roll.total == i), ];
  
  roll_12_orless_outcome = outcome[1, 'prob'];
  
  roll_12_orless_1x = roll_12_orless_1x + roll_12_orless_outcome;
  #print(i)
}
cat(roll_12_orless_1x, "\n");
...
ANSWER: The probability that you roll at most a 12 on a single throw is 0.22.
...
ANSWER: The histogram shows that the number of jobs for specific search queries is heavily weighted under 50,000. The box plot with its concentration near the bottom, shows essentially the same thing; < 50k job search query results, with a few outliers near the top. Finally, the summary provides the mean, median, min, max, and 1st & 3rd quartiles, which show that the data set does not have a symmetrical distribution.
...
# histogram
graphics::hist(jobs.subset$job.count, breaks = 20);

#boxplot
graphics::boxplot(jobs.subset$job.count);


# summary
base::summary(jobs.subset$job.count);
...
ANSWER: The histogram here is showing that there are only a minimal number of keywords searched above 100,000, and even less above 200,000, with the vast majority showing up in the < 100,000, and especially the < 50,000 ranges.The boxplot is again cocentrated toward the bottom of the visual, indicating the aforementioned results, with outlier extending out above the box.
...
ANSWER: The parallel-ish lines indicate that the rate at which the frequency of the keywords are queried is steady with a low degree of variability from week to week.

Data Analysis is trending much higher than other words save Javascript which is following Data Analysis comparatively closely. And even then, the next words are Data Analytics and Machine Learning, both terms closely related to Data Analyics. 

My perspective from the above is that we can determine that Data Analysis is a very popular term. However all terms have comparable rates of increasing frequency with the relatively short time frame that this data set covers, so perhaps a sample covering a longer period of time might be worth exploring to reveal broader trends.
...
ANSWER: the Git data is mostly likely simply missing some values for the weeks that it dipped, the alternative explanation, that there were suddenly zero results for that term during that period is not a reasonable conclusion as employers would not suddenly have no need for Git related skills, one week, then do so again the following week. 
...
Yes, as the two boxplots do not overlap at all we can conclude that their individual interquartile ranges completely differ based on both their boxplots and mathematics.

From a mathematics perspective the lack of overlap between the boxplots as a representation of numerical values enables us to employ parallel-line logic to subtract the lower from the higher resulting in the difference. We can see that Micro$oft Office has a low of about 225 and C++ has a high of about 170, this is as close as the two come to meeting

One formal inferential statistical test we could use here is a two sample t-test.

If not from mathematics and EDA formal tests may have been derived from analyzing data samples in the goal was to confirm hypothesis already proven. "Backtesting?"
...
t.test.jobs(jobs.subset, "Java", "Big Data");
...
This is an interesting case here with Java and Statistics, because looking at the box plots now we actually do have a large degree of overlap, to the point, where we can say based on the boxplots that there is no significant difference between their individual job count.

Mathematically, however the case is different, as there is clearly distance between their lines, indicating that they are not likely to have the same job count.

Finally, switching out the values for C++ and Microsoft Office in the Welch Two Sample t-test code block above allows us to run the same test on Java and Statistics which gives us a p-value of 0.6333, which is greater than the significance level of 0.05. thus we can conclude that there is no difference between the means of Statistics and Java.
</pre>
...
There's a good degree of overlap with the boxplots. The Data Science median is equal to the Big Data first quartile, while both third quartiles are very close. This suggests that Big Data is less normally distributed than Data Science.

Mathematically, there's clearly some degree of distance using parallel-line logic. 

Finally, t-test attempt threw an error, apparently Data Science and Big Data do not have enough observations to properly preform the test. 

I would say, based on my still forming, but rapidly expanding thanks to this class, intuition that it is likely that a single-job posting may have both keywords.
...
Data provenance is essentially establishing a high standard for the foundational aspects of the data analysis process in order to reduce the negative impacts of poorly handled data on results. It includes data collection, cleaning, sand boxing, establishing questions, evaluation, report drafts, visualizations, and final report. This entire process must be approached with the utmost care and an emphasis on meticulous documentation of all steps. Data Provenance is a broad over-arching approach to each specific aspect of the data analytics workflow, in which the analyst takes a great deal of care to perform each step with a high degree of emphasis on quality assurance, executing each point with an eye to make it the most pristine it can be.

One big milestone in Data provenenance is the point when the data has been prepared enough to allow the analyst to begin developing a set of initial questions related to the data. This step requires some open-mindedness and thorough thinking of the data from multiple perspectives. It can be the case that data does not the support answering one of the initial questions, which is why this step goes through multiple revisions. 

Creating visualizations to present your findings is the next major step. A key concept for this step is “the power of one” in which a single graphic, of high quality, succinctly and expertly communicates the entire anaysts interpetation of the data to an audience of non-scientists.
...
The box is not a square due to the Mercator projection, a map of the earth that became the standard for navigation after it was invented in 1569. As a byproduct of the maps conformality, land masses further away from the equator are presented as larger than their true to life size relative to other land masses closer to the equator.

I find the visualization a bit bland and uninspired, I would perhaps probably employ the use of various map related packages to add additional data, enhance color pallete and maybe even combine it with other plots.
...
hometown.latitude = 47.337193; 
hometown.longitude = -122.584930;
my.radius = 13; my.units = "mi"; #miles

hometown.info = getNeighborsFromLatLong(47.337193, -122.584930, "mi");
...
hometown.info = getNeighborsFromLatLong(13, 47.337193, -122.584930, "mi");
...
The ggplot map is the most visually appealing to me, I'd say because it's the one that looks the most like an actual map. 

For functionality, I would again say that I prefer the ggplot map. In Data provenance you discussed the importance of presenting data to a "general audience". I think the ggplot map does this better than the other two because the voroni map is perhaps to complicated for a general audience, I can see people having trouble visually parsing what they're looking at. The base R map is just too minimalist.

The physical size of the map you're generating could determine whether or not to put Alaska and Hawaii next to California, if it's a smaller, more compact map, putting them next to California is a good way to save space.
...
The graph would suggest a linear relationship between Haversine and Meeus.

Conceptually, Meeus is "the shortest distance between two points on an ellipsoid", while Haversine is the "the shortest distance between two points while ignoring ellipsoidal effects."

The results are similar, however is generally considered superior in most cases.

A more accurate geo-spatial distance algorithm is distGeo.
...
The relationship between these two is less similar, the distribution is weighted on the higher end, with only a single outlier on the low end.

They share some degree of similiarity, however it is less than with Meeus and Haversine.
</pre>
[
...
After looking at both, I chose to go with ward2 rather than complete. Overall this appears to be a reasonable method of clustering the state capitols and does align with my own preconceived notions I would say, though my knowledge of the pacific northwest far exceeds any other region of the country.

I do see North and South Dakota together and the Carolinas. I was a bit surprised to see Nevada clustered with the rest of the pacific northwest but upon further reflection I can see why that was done. Kentucky and Indiana are together. I also found it peculiar not to see Illinois with them as well since the three are part of a tri-state area.
...
# (1) populate cluster.arbitrary (Maybe just skip this......)
...
Wow! This is a gorgeous graphic. I really like a colorful it is and the slight cartoony art style, my favorite bit is the sun with a face.

I was a little thrown by the darker blue in the Montana graph.

I found it to be very aesthetically pleasing, very vibrant.

It is functional, I was able to read and understand the data it presents relatively quickly and with little effort.
...
I like the increased functionality of this graphic. Having to switch back and forth between the two before was annoying.

It does appear to be a bit of a tight fit though, so I would say that is one minor dislike.

I do find it aesthetically pleasing.

I would say this is even more functional than the previous.

The y-axis are NOT the same scale and the visible grid lines aren't the same either. Montana peaks at 100 degrees F while Lousiana goes up to 120 degrees F. Also Montana's gride lines begin at -40 degrees F, while Louisiana's start at 0 degrees F (bit of a problem there).

The snow is a darker blue shape that looks like icecicles while the rain is a lighter blue that looks like rain droplets. I feel the snow part is a tad confusing. It's reasonable that the snow and rain are close in color but perhaps it doesn't work  the best for a visually. I may have attempted some off-white palettes to represent the snow.
...
-- CLUSTERS -- |        --  STATES --
Cluster 1: Montgomery, AL, Juneau, AK 
Cluster 2: Phoenix, AZ
Cluster 3: Little Rock, AR
Cluster 4: Sacramento, CA, Denver, CO, Hartford, CT
Cluster 5: Dover, DE, Tallahassee, FL, Atlanta, GA, Honolulu, HI, Boise, ID, Springfield, IL, Indianapolis, IN, Des Moines, IA, Topeka, KS, Frankfort, KY, Baton Rouge, LA	5, Augusta, ME, Annapolis, MD	
Cluster 6: Annapolis, MD, Boston, MA, Lansing, MI, Saint Paul, MN, Jackson, MS, Jefferson City, MO, Helena, MT, Lincoln, NE	
Cluster 7: Carson City, NV
Cluster 8: Concord, NH, Trenton, NJ, Santa Fe, NM, Albany, NY, Raleigh, NC, Bismarck, ND	
Cluster 9: Columbus, OH
Cluster 10: Oklahoma City, OK, Salem, OR, Harrisburg, PA, Providence, RI
Cluster 11: Columbia, SC, Pierre, SD, Nashville, TN, Austin, TX, Salt Lake City, UT
Cluster 12: Montpelier, VT, Richmond, VA, Olympia, WA, Charleston, WV, Madison, WI, Cheyenne, WY

The Primary Characteristic of all clusters is the state alphabatized into the clusters.
...
July in record high F (C) has a trend showing a correlation with the seasonal time of year of any month in relation to itself. For months that are closest to July (ex: June (0.90)) the correlation is strong, for months that are further away from July (ex: January (0.25)) the correlation is much weaker. Both correlations are positive, however. Overall this suggests that months closer to July during the year are much more likely to be like July.

Latitude, as a measurement of north/south points perpendicular to the equator,  correlates with temperature as a product of season. Latitude is a measurement of north/south in relation to the equator, thus a point exactly at the equator would have a latitude value of 0 and a point exactly at the north or south pole would have a latitude value of 90. This correlation is negative which means for latitude values that are higher, or further away from the equator the temperature (as shown by the month of the year) is lower (ex: July (-0.15)), and latitude values that are lower, or closer to the equator the temperature (as shown by the month of the year) is higher (ex: January (-0.83)). Both are strong, suggesting that Latitutde is a very good predictor of temperature.

July in record high F (C) has very little correlation with Latitude at -0.15. What correlation there is, is negative, indicating that there is a weak, negative relationship with Latitude.

Longitude, as a measure of east/west, has no correlation with temperature (ex: July (0.02)). This relationship is weak to non-existant, meaning that Longitude is a poor predictor of temperature.

July in record high F (C) has even less correlation with Longitude than with Latitutde, with Longitude being at 0.02. This correlation is almost non-existant, and is positive, indicating nothing really.

Population has a moderate correlation with season (and thus temperature). This correlation is positive meaning that as population density increases so does temperature. This correlation is a moderately strong (ex: 0.31) indicating that there is a moderate relationship between population and temperature.

July in record high F (C) has a moderate correlation with Population. This correlation is positive and only moderate at 0.31, indicating that there is somewhat of a relationship between the two.

I think the winter of December and January correlate the most with Latitude and the summer months July and August correlate the least. Yes the correlation is alway the same sign (negative).
</pre>
...
January in record low F (C) has a trend showing a correlation with the seasonal time of year of any month in relation to itself. For months that are closest to January (ex: December (0.97)) the correlation is strong because they are very much alike, for months that are further away from January (ex: July (0.74)) the correlation is weaker but not by much. Both correlations are positive, however. Overall this suggests that months closer to January during the year have a strong relationship and are alike, while months further away from January like July have a somewhat weaker relationship.

Latitude, as a measurement of north/south points perpendicular to the equator,  correlates with temperature as a product of season. Latitude is a measurement of north/south in relation to the equator, thus a point exactly at the equator would have a latitude value of 0 and a point exactly at the north or south pole would have a latitude value of 90. This correlation is negative which means for latitude values that are higher, or further away from the equator the temperature (as shown by the month of the year) is lower, and latitude values that are lower, or closer to the equator the temperature (as shown by the month of the year) is higher. Both are strong and consistent throughout the year, suggesting that Latitude is a very good predictor of temperature.

January in record low F (C) has a strong, negative correlation with Latitude at -0.72. This indicates a strong, inverse relationship between January and Latitude. 

Longitude, as a measure of east/west, has a minor correlation with temperature (ex: January -0.33. This relationship is weak, and get's weaker throughout the year, meaning that Longitude is a poor predictor of temperature. The correlation that there is ranges from negative to positive, indicating a slight increases or decreases in both or either.

January in record low F (C) has somewhat of a correlation with Longitude at -0.33. This correlation is not strong, verging on weak and is negative, indicating that January and Longitude have an inverse relationship and when one goes up, the other may slightly go down.

Population has a moderate correlation with season (and thus temperature). This correlation is positive meaning that as population density increases so does temperature. This correlation is a moderately strong (ex: 0.31) indicating that there is a moderate relationship between population and temperature.

January in record low F (C) has a minor correlation with Population, about on par with Longitude (maybe they can be friends!) This correlation is positive and only moderate at -0.34, indicating that there is somewhat of a positive linear relationship between January and Population suggesting that when one goes up the other is slightly likely to go up as well.

I think the summer months of July and August have the highest correlation at -0.8, this correlation is also negative, indicating that when one goes up the other goes down. I'm not sure about the least, I can see that the months of February and October have the least correlation, but are still pretty high at -0.64. This correlation is negative indicating that when one goes up the other goes down. Yes, the correlation is always the same sign (negative).
...
x = c(25,25,15,15,10,10);  ## change these values and discuss ...
x.labels = c("collection", "organization", "EDA analysis", "CDA analysis", "interpretation", "presentation");
x.colors = c("blue", "lightgreen", "green", "darkgreen", "orange", "red");
...
Data collection is one of the most important, if not the most important steps in data analysis. This is because all other steps are built on its foundation. Improper data collection methods ensure the steps that follow are all but meaningless. For example, stellar analysis of the data or an amazing presentation of the data will not fix the errors created by poor data collection, the work is tainted by this “bad data” and the only solution is to re-collect the data properly. For this reason I would suggest that Data collection be valued at 25%. 

The importance of the data organization step is often overlooked, however data wrangling or data munging to a data analyst is a critical step in much the same way as data collection. Data that is improperly wrangeld yields a corrupt dataset filled with holes upon which any further analysis or knowledge gleaned cannot be seen as reliable. Given this I would assign the data organization step a value of 25%.

The analysis step is perhaps the most technical step in the data analysis workflow. Requiring the most expertise and thus resources, while also having a large ability to render the project a failure. Examining this step in terms of Exploratory Data Analysis and Confirmatory Data Analysis we see support for the importance of both. Confirmatory Data Analysis is obviously important because confirmation of specific hypotheses is a critical step in being able to reliably apply knowledge gleaned from analysis. Exploratory Data Analysis, on the other hand, provides the opportunity for the discovery of previously unknown patterns in data. EDA and CDA work best in tandem, with one following the other, though it’s important to not use the same dataset for both, since EDA has the potential to suffer from a Type 1 error, which could then show up in the CDA as well, if the same dataset were used. Given the importance of the analysis step, inclusive of both EDA and CDA, I would assign it a value of 30%.

The interpretation step in the process is certainly an important one, after all, what good is a poor interpretation of analysis as it will more likely not provide usable knowledge? However, an improper interpretation of the results of analysis is far easier rectified than improper data collection, organization, or analysis, it is for this reason that I assign interpretation a value of 10%.

The final step, presentation is the culmination of all of the work that came before it, it is the moment when said can be deemed of value, based on the quality of the presentation and whether or not the results and implications of the analysis are communicated in a useful manner. However as will the interpretation step, a shoddy presentation, while having the ability to be quite damaging if it is misleading, can be quite easily rectified, and its contingence on all previous steps places them higher on the hierarchy of importance, thus I would assign the presentation step a value of 10%.

I would consider there to be two categories of “Tools of the Trade” for a Data Analyst. The access and working knowledge of software tools, such as R, and applied knowledge of tools such as how to perform a T-test, etc. The software tools that I think are important are to have a functional knowledge of Python, R, SQL, Tableau, Minitab, Excel, Hadoop, and perhaps SPSS to name the most fundamental. Reporting metrics sources such as Google Analytics. One should also have the ability to apply foundational computer science principles and general problem solving and program design, an array of the standard statistical tests such as the T-test, Analysis of Variance, and Linear Regression. Also knowledge of data science algorithms such apriori, and knowledge of machine learning. Some knowledge of linear algebra’s coverage matrices and related topics is also helpful. A good knowledge of the data science workflow is also important. I could certainly have a better understanding of R but I’m fairly comfortable with the software tools. In order of proficiency I’d say my understanding hierarchy would be Python > Excel > R > Tableau > Minitab > SQl > Google Analytics > Hadoop > SPSS. I included SPSS even though it goes untouched in most college programs and has arguably been eclipsed by other software, because it remains in use by many companies in the business world. For knowledge skills I have a decent foundation in basic computer science and problem solving coming from that major. As for statistical test knowledge I can perform them adequately enough, though I think I need to brush up. Algorithms and machine learning would be somewhat below my statistical knowledge, though  The one area I’m really focused on wrapping my head around is the Data Science workflow as I’m reading that it’s critically and important and further that once one has a good handle on it, the various other tools cement themselves much easier.

Skills of the trade

Data Intimacy: Emerging - The attention and passion paid to the collection of the data being on par with the attention and passion paid to the analysis of the data is a skill I feel that I have not yet begun to fully realize. I base this opinion on the fact in my course work I’ve found myself doing comparatively little data collection myself, in fact I would definitely say that the Data Analysis curriculum at WSU is weighted heavily toward performing data analysis (perhaps there may be some benefit to bringing this issue up with the heads of the department). This is one of the reasons I appreciated the measure project we’ve worked on in this class. I was apprehensive at first, about having to collect my own data, but having to work it out and develop a method to do it was actually one of the better learning experiences I’ve had at this university. So while I still feel that my skill in this area is emerging, I can see the road ahead on how to advance it much clearer now.

Track/Document Data Curation: Emerging - while I have some level of knowledge about the tracking and curation of data, I would definitely say that I am not yet at developing.

Track/Document/Recreate/Version Control Analysis: Emerging-Developing - there’s certainly a lot that goes into this skill of the trade and a lot more I need to learn, however I feel that my skills are above emerging but still below developing, as I can record and repeat my analysis’. 

Perspectives: Emerging - I feel that like Data Intimacy this is a beyond basic skill, given my hesitance and current lack of autonomy in interpretation I feel that the ability to view data from multiple perspectives has not yet reached developing. I want to take some time to perform some projects on my own, using a full data science workflow first. 

Visualizations: Emerging - I feel that I’m closer to developing on this skill than the previous one, however I’m still not confident enough in it to say that my level is at emerging 

Communication: Developing - As more of an extrovert, I’d say if there are any skills of the trade that come “naturally” to me, it would be communication. However, tailoring those skills to make them specific to data analysis presentation is still very much an ongoing learning experience.

As I spoke about in a post on the class forum, I appreciate the way this class is structured in a way to promote the enhancement of skills that are required in a real-world job environment. I can say without a doubt that this is the most difficult class I’ve taken at WSU, but Benjamin Franklin said that “from adversity comes opportunity” and I think that’s true of this class as it sharpens us for the job market, better preparing us to seize the opportunities that lay ahead.
...
# you can c5hange col ... cex (font size), srt (angle), and pos = 1,2,3,4
# 
points(0.85, 0.95, pch=20, col="#981e32");
text(0.85, 0.90, "Michael (self)", col="#981e32", cex=0.75, srt = 45, pos=4);
# 
# ## evaluate the Course Categories of Tools of the Trade
# ## give them a score
# 
points(-0.55, 0.55, pch=20, col="#8f7e35");
text(-0.55, 0.55, "Math(s)", col="#8f7e35", cex=0.5, srt = 45, pos=2);
# 
# 
points(0.15, 0.50, pch=20, col="#b67233");
text(0.15, 0.50, "Computer Science", col="#b67233", cex=0.5, srt = 45, pos=3);
# 
points(0.95, 0.80, pch=20, col="#4f868e");
text(0.95, 0.80, "Machine learning", col="#4f868e", cex=0.5, srt = 45, pos=4);
# 
points(0.00, 0.85, pch=20, col="#464e54");
text(0.00, 0.85, "Statistics", col="#464e54", cex=0.5, srt = 45, pos=1);
# 
points(0.70, 0.80, pch=20, col="#5e6a71");
text(0.70, 0.80, "Data analytics", col="#5e6a71", cex=0.5, srt = 45, pos=2);
# 
# # You have a track (e.g., Business)
points(0.30, 0.05, pch=20, col="#4f868e");
text(0.30, 0.05, "Business", col="#4f868e", cex=0.5, srt = 45, pos=2);
